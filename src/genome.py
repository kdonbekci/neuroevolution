...
from gene import InputGene, OutputGene, NodeGene, ConnectionGene, PseudoGene
...
class Genome:
    ...
    def initialize(self, inputs, outputs, output_activation, loss, _id, generation):
        ...
        for i in inputs:
            g_in = InputGene()
            g_in.initialize(shape=i, generation=self.origin)
            self.add_gene(g_in)
...
#         assert gene.inno_num not in self.genes #temporary
    ...
    @staticmethod
    def crossover(_id, more_fit_parent, less_fit_parent, generation):
        child = Genome(_id)
    ...
    def copy(self, maintain_bias=False, maintain_weights=False, maintain_incoming_outgoing=True):
        ...
        clone.inputs = self.inputs.copy()
        clone.outputs = self.outputs.copy()
        clone.genes = {}
        for i in self.genes:
